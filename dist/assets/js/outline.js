...
(function () {
  ...
  let dragFrom = null;
  let activeVol = D.current.volumeId && D.volumes.some(v => v.id === D.current.volumeId)
    ? D.current.volumeId : (D.volumes[0] && D.volumes[0].id);
    ...
    vol.beats.forEach((b, i) => {
      ...
      const card = APP.el(`
        <article class="beat-card ${b.id === activeBeat ? 'active' : ''}" data-b="${b.id}" draggable="true" style="${b.id === activeBeat ? 'box-shadow:0 0 0 3px rgba(232,155,84,.2),var(--shadow-card)' : ''}">
          ...
          <div class="row3">
            ...
            <span class="spacer"></span>
            <button class="chip" data-up title="上移" ${i === 0 ? 'disabled' : ''}>↑</button>
            <button class="chip" data-down title="下移" ${i === vol.beats.length - 1 ? 'disabled' : ''}>↓</button>
      ...
      card.addEventListener('click', e => {
        ...
        if (e.target.closest('[data-up]')) {
          APP.store.moveBeat(activeVol, i, i - 1);
          renderVols(); renderBeats();
          APP.toast('已上移至第 ' + (i) + ' 章', 'success');
          return;
        }
        if (e.target.closest('[data-down]')) {
          APP.store.moveBeat(activeVol, i, i + 1);
          renderVols(); renderBeats();
          APP.toast('已下移至第 ' + (i + 2) + ' 章', 'success');
          return;
        }
        activeBeat = b.id;
      ...
      card.addEventListener('dragstart', e => {
        ...
        try { e.dataTransfer.setData('text/plain', String(i)); } catch (err) { /* IE 兼容 */ }
      });
      ...
      card.addEventListener('drop', e => {
        ...
        APP.store.moveBeat(activeVol, from, i);
        renderVols(); renderBeats();
        APP.toast('已调整章节顺序', 'success');
      });
  ...
  function renderDetail(b, idx) {
    ...
    pane.innerHTML = `
      ...
      ${branches.length ? `<div class="sec"><div class="st">剧情分支</div><div class="fs-list">${branches.map((x, bi) => `<div class="fs-item"><span class="dot" style="background:var(--lilac)"></span><span style="flex:1">${APP.esc(x)}</span><button class="chip" data-adopt="${bi}">采纳</button></div>`).join('')}</div></div>` : ''}
      <div class="sec">
        <div class="st">校验</div>
        <div class="fs-list">
    ...
    /* 采纳分支：写入本章概要 */
    pane.querySelectorAll('[data-adopt]').forEach(btn => {
      btn.addEventListener('click', () => {
        const bi = +btn.dataset.adopt;
        const branch = branches[bi];
        if (!branch) return;
        if (!confirm('将该分支采纳为本章概要？\n\n' + branch)) return;
        APP.store.updateBeat(activeVol, b.id, { sum: branch });
        b.sum = branch;
        renderBeats(); renderDetail(b, bi + 1);
        APP.toast('已采纳该分支为本章概要', 'success');
      });
    });
  }
...
要求：每章一行，格式为「章节标题 —— 一句话概要」。世界观为奇幻治愈，细腻慢热叙事。`, {
  ...
  document.getElementById('btn-add-beat').addEventListener('click', () => {
    if (!activeVol) { APP.toast('请先新建一个分卷', 'warn'); return; }
    APP.modal({
      title: '新建章节大纲',
      submitText: '创建',
      bodyHtml: `
        <div class="field"><label>章节标题</label><input class="input" name="title" placeholder="例如：银月之井"></div>
        <div class="field"><label>一句话概要</label><textarea class="textarea" name="sum" rows="2" placeholder="本章要发生什么"></textarea></div>
        <div class="field"><label>出场人物（逗号分隔）</label><input class="input" name="chars" placeholder="凛, 陆沉"></div>
        <div class="field"><label>埋设伏笔（逗号分隔）</label><input class="input" name="fs" placeholder="碎星盏"></div>`,
      onSubmit: (root) => {
        const f = APP.formData(root);
        if (!f.title.trim()) { APP.toast('请填写章节标题', 'warn'); return false; }
        const b = APP.store.addBeat(activeVol, {
          title: f.title.trim(),
          sum: f.sum.trim(),
          chars: f.chars.split(/[,，]/).map(x => x.trim()).filter(Boolean),
          fs: f.fs.split(/[,，]/).map(x => x.trim()).filter(Boolean)
        });
        activeBeat = b && b.id;
        renderVols(); renderBeats();
        APP.toast('章节大纲已创建', 'success');
        return true;
      }
    });
  });
